...
## 面试题中的算法题注意点
### 时间复杂度
计算时间复杂度，时间复杂度就是语句执行的次数，所以关系着性能，时间复杂度分为以下几点：
1.常数：就是不存在循环的情况，那么他的时间复杂度为O(1)
2.线性：就是存在一层循环体，常见的是遍历数组的情况，所以时间复杂度为O(n)
3.对数：就是执行次数为N的一半的情况，例如折半查找，二叉树查询，时间复杂度为O(logN)
4.平方：就是存在两层循坏的情况，例如冒泡排序，选择排序
